...
TEST_CASE("Markup Auto Close Tag", "[jot]")
{
  ...
  REQUIRE(closing == "</div>");
  REQUIRE(HtmlFeatures::should_insert_closing_tag("return <Component>", 18, closing));
...
TEST_CASE("Markup Between Tags", "[jot]")
{
  ...
  REQUIRE(tag == "div");
  REQUIRE(HtmlFeatures::is_between_matching_tags("return <Component>", "</Component>", tag));
...
{
  auto range = QuoteTextObject::find_inner_range("s = \"a \\\"quoted\\\" value\";", 8, '"');
  REQUIRE(range.found);
...
{
  auto range = QuoteTextObject::find_inner_range("\"left\" + \"right\"", 8, '"');
  REQUIRE(range.found);
  REQUIRE(range.open == 9);
  REQUIRE(range.close == 15);
}
