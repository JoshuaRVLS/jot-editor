...
TEST_CASE("Real Lua API bootstrap exposes jot.treesitter.native") {
  ...
  REQUIRE(lua_istable(L, -1));
  for (const char *fn : {"register_language", "language_for_extension",
                         "disable_language", "install_command", "status",
                         "parser", "parse", "query", "captures",
                         "set_query", "set_capture_color", "reload"}) {
    lua_getfield(L, -1, fn);
