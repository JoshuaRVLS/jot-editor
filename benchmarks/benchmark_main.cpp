...
namespace
{
  ...
  std::vector<BenchmarkCase> make_benchmarks()
  {
    ...
    return {
        ...
        {"lazy_line_provider_index_and_reads",
         5,
         [cpp_lines]()
         {
        ...
        {"folding_visible_navigation",
         ...
         [cpp_lines]()
         {
           ...
           {
             line = Folding::advance_visible_lines(ranges, line, 1, (int)cpp_lines.size());
           }
           if (line < 0)
           {
        ...
        {"symbol_index_python_document",
         ...
         [python_lines]()
         {
           auto symbols = SymbolIndex::extract_document_symbols(python_lines, "module.py");
           if (symbols.empty())
           {
             throw std::runtime_error("expected python symbols");
           }
         }},
        {"workspace_search_temp_project",
         5,
         [cpp_lines]()
         {
           ...
           for (int i = 0; i < 40; i++)
           {
             ...
             write_lines(subdir / ("file_" + std::to_string(i) + ".cpp"), cpp_lines);
           }
           auto results = WorkspaceSearch::search(
               temp.path().string(), "function_11999", 4 * 1024 * 1024, 2000);
           if (results.empty())
           {
  ...
  try
  {
    ...
    for (const auto &bench : benches)
    {
      ...
      std::cout << std::left << std::setw(36) << result.name << "  "
                << "iters=" << std::setw(3) << result.iterations << "  " << std::fixed
                << std::setprecision(3) << "min=" << std::setw(9) << result.min_ms << "ms  "
    ...
    std::cout << "\nsummary_avg_ms=" << std::fixed << std::setprecision(3) << total_avg << "\n";
    return 0;
  }
