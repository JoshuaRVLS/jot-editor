...
namespace
{
  ...
  CaptureStyle capture_style_for_name(std::string_view sv)
  {
    ...
    if (sv == "comment" || has_prefix("comment."))
      return {TS_TOKEN_COMMENT, 120};
    if (sv == "string.escape" || sv == "escape")
    {
      return {TS_TOKEN_STRING_ESCAPE, 118};
    }
    if (sv == "string" || has_prefix("string.") || sv == "character" || has_prefix("character."))
    {
    ...
    if (sv == "function" || has_prefix("function.") || sv == "method" || has_prefix("method.")
        || sv == "constructor")
    {
      ...
      if (sv == "function.method" || sv == "method" || has_prefix("method."))
      {
    ...
    if (sv == "variable.parameter" || sv == "parameter" || has_prefix("parameter."))
    {
      return {TS_TOKEN_PARAMETER, 90};
    }
    if (sv == "variable.member" || sv == "property" || has_prefix("property.") || sv == "field"
        || has_prefix("field."))
    {
      return {TS_TOKEN_FIELD, 88};
    }
    if (sv == "attribute" || has_prefix("attribute.") || sv == "tag.attribute")
    {
    ...
    if (sv == "keyword.storage" || has_prefix("keyword.storage.") || sv == "keyword.modifier"
        || has_prefix("keyword.modifier."))
    {
      return {TS_TOKEN_KEYWORD_STORAGE, 78};
    }
    if (sv == "keyword.directive" || has_prefix("keyword.directive.") || sv == "keyword.import"
        || sv == "preproc" || has_prefix("preproc."))
    {
    ...
    if (sv == "number" || has_prefix("number.") || sv == "float" || sv == "constant.numeric")
    {
      return {TS_TOKEN_NUMBER, 72};
    }
    if (sv == "constant.builtin" || sv == "boolean" || sv == "none" || sv == "null")
    {
      return {TS_TOKEN_BUILTIN, 70};
    }
    if (sv == "constant.macro" || sv == "constant.predefined" || sv == "constant.define")
    {
...
int tree_sitter_capture_color_for_name(const std::string &name)
{
  ...
  if (it != capture_color_overrides.end())
    return it->second;
...
void SyntaxHighlighter::set_language(const std::string &ext)
{
  ...
  {
    // Keywords
    rules.push_back({std::regex("\\b(int|char|void|float|double|bool|long|short|unsigned|"
    ...
    rules.push_back(
        {std::regex("#\\s*include\\s*[<\"][^>\"]+[>\"]"), 6}); // Cyan for entire include
    rules.push_back({std::regex("#\\s*[a-zA-Z_]+"), 5});       // Magenta for #define, #ifdef, etc.

    rules.push_back({std::regex("\"([^\"\\\\]|\\\\.)*\""), 2});
    rules.push_back({std::regex("'([^'\\\\]|\\\\.)*'"), 2});
    rules.push_back({std::regex("//.*"), 3});
    rules.push_back({std::regex("/\\*.*\\*/"), 3}); // single-line block chunk
    rules.push_back({std::regex("\\b(0x[0-9a-fA-F]+|0b[01]+|\\d+\\.\\d+([eE][+-]?\\d+)?|"
                                "\\d+[eE][+-]?\\d+|\\d+)\\b"),
                     4});
    rules.push_back({std::regex("\\b[A-Za-z_][A-Za-z0-9_]*\\b(?=\\s*\\()"),
  ...
  else if (ext == ".py")
  {
    ...
    rules.push_back({std::regex("\\b(def|class|if|elif|else|for|while|return|try|"
    ...
    rules.push_back(
        {std::regex("\"\"\".*\"\"\"|'''.*'''|\"([^\"\\\\]|\\\\.)*\"|'([^'\\\\]|\\\\.)*'"), 2});
    rules.push_back({std::regex("#.*"), 3});
    rules.push_back({std::regex("\\b(0x[0-9a-fA-F]+|\\d+\\.\\d+([eE][+-]?\\d+)?|\\d+)\\b"), 4});
    rules.push_back({std::regex("@[a-zA-Z0-9_]+"), 6});                          // Decorators
    rules.push_back({std::regex("\\b[A-Za-z_][A-Za-z0-9_]*\\b(?=\\s*\\()"), 6}); // Calls
  }
  else if (ext == ".js" || ext == ".ts" || ext == ".jsx" || ext == ".tsx" || ext == ".mjs"
           || ext == ".cjs")
  {
    rules.push_back({std::regex("\\b(import|from|export|require)\\b"), 5}); // Imports

    rules.push_back({std::regex("\\b(var|let|const|function|return|if|else|for|while|do|switch|"
  ...
  else if (ext == ".rs")
  {
    ...
    rules.push_back({std::regex("\\b(fn|let|mut|const|static|struct|enum|impl|trait|type|pub|"
  ...
  {
    rules.push_back({std::regex("\\b(body|div|span|h[1-6]|p|a|ul|ol|li|table|tr|td|th|form|input|"
  ...
  else if (ext == ".java" || ext == ".kt")
  {
    ...
    rules.push_back({std::regex("\\b(public|private|protected|class|interface|enum|extends|"
  ...
  else if (ext == ".go")
  {
    ...
    rules.push_back({std::regex("\\b(func|type|struct|interface|map|chan|go|"
  ...
  {
    rules.push_back({std::regex("\\b(if|then|else|elif|fi|case|esac|for|while|until|do|done|"
  ...
  else if (ext == ".rb")
  {
    ...
    rules.push_back({std::regex("\\b(def|end|class|module|if|else|elsif|unless|while|until|for|in|"
  ...
  else if (ext == ".php")
  {
    ...
    rules.push_back({std::regex("\\b(php|echo|function|class|public|private|protected|static|if|"
  ...
  {
    rules.push_back({std::regex("\\b(local|function|end|if|then|elseif|else|for|while|repeat|"
  ...
  {
    rules.push_back({std::regex("\\b(import|class|struct|enum|protocol|extension|func|let|var|"
  ...
  else if (ext == ".cs")
  {
    ...
    rules.push_back({std::regex("\\b(public|private|protected|internal|class|struct|interface|"
  ...
  {
    rules.push_back(
        {std::regex("\\b(select|insert|update|delete|from|where|join|left|right|inner|"
  ...
  {
    rules.push_back({std::regex("\\b(if|else|elseif|endif|foreach|endforeach|while|endwhile|"
  ...
  {
    rules.push_back({std::regex("^(from|run|cmd|entrypoint|env|arg|workdir|copy|add|expose|user|"
  ...
  else if (ext == ".make" || ext == ".mk")
  {
    ...
    rules.push_back({std::regex("\\b(ifdef|ifndef|ifeq|ifneq|else|endif|include|define|endef|"
                                                               ...
                                                               int byte_limit)
{
  ...
  auto apply_rule = [&](const SyntaxRule &rule, bool protect_only, bool skip_protected)
  {
    const std::string &src = (scan == n) ? line : line.substr(0, (size_t)scan);
