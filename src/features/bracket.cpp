...
std::pair<int, int>
BracketHighlighter::find_matching_bracket(const std::vector<std::string> &lines, int line, int col)
{
...
std::vector<BracketPair> BracketHighlighter::find_all_pairs(const std::vector<std::string> &lines)
{
    ...
    for (size_t j = 0; j < lines[i].length(); j++)
    {
      ...
      else if (is_closing_bracket(ch))
      {
        if (!stack.empty())
        {
          ...
          if (open_pos.y >= 0 && open_pos.y < (int)lines.size() && open_pos.x >= 0
              && open_pos.x < (int)lines[open_pos.y].length()
...
int BracketHighlighter::get_bracket_color_at(const std::vector<std::string> &lines,
                                             int line,
                                             int col,
...
std::pair<int, int> BracketHighlighter::find_matching_bracket_near_cursor(
    const std::vector<std::string> &lines, int cursor_line, int cursor_col)
{
  if (cursor_line < 0 || cursor_line >= (int)lines.size())
    return {-1, -1};

  int check_positions[][2] = {
      {cursor_col, cursor_line}, {cursor_col - 1, cursor_line}, {cursor_col + 1, cursor_line}};
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    if (line >= 0 && line < (int)lines.size() && col >= 0 && col < (int)lines[line].length())
    {
