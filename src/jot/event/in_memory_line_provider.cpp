...
InMemoryLineProvider::InMemoryLineProvider()
{
  lines_.push_back("");
}
...
char InMemoryLineProvider::get_char(int line, int col) const
{
  if (line < 0 || line >= (int)lines_.size())
    return '\0';
  if (col < 0 || col >= (int)lines_[line].size())
    return '\0';
  return lines_[line][col];
}

size_t InMemoryLineProvider::line_count() const
{
  return lines_.size();
}
...
void InMemoryLineProvider::insert_lines(int after, const std::vector<std::string> &lines)
{
...
void InMemoryLineProvider::move_lines(int start, int end, int dest)
{
  ...
  if (dest > start)
  {
    std::rotate(lines_.begin() + start, lines_.begin() + end + 1, lines_.begin() + dest + 1);
  }
  else
  {
    std::rotate(lines_.begin() + dest, lines_.begin() + start, lines_.begin() + end + 1);
  }
}

void InMemoryLineProvider::replace_lines(int start,
                                         int count,
                                         const std::vector<std::string> &new_lines)
{
  ...
  if (start < 0)
    start = 0;
  if (start > (int)lines_.size())
    start = (int)lines_.size();
