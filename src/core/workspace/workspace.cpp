...
namespace
{
  void flatten_nodes_mut(std::vector<FileNode> &nodes, std::vector<FileNode *> &flat)
  {
    for (auto &node : nodes)
    {
      flat.push_back(&node);
  ...
  void flatten_nodes_const(const std::vector<FileNode> &nodes, std::vector<const FileNode *> &flat)
  {
