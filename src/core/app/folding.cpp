...
namespace
{
                                 ...
                                 int line_count)
  {
    if (desired_row < 0)
    {
      return Folding::clamp_scroll_offset(ranges, header_line, viewport_h, line_count);
    }
    ...
    int scroll = Folding::buffer_line_for_visible_index(ranges, target_visible_index, line_count);
...
bool Editor::toggle_fold_at_line(FileBuffer &buf, int line)
{
  ...
  if (will_collapse)
  {
    buf.scroll_offset = anchored_scroll_for_header(
        buf.fold_ranges, header_line, header_row, viewport_h, (int)buf.line_count());
  }
  needs_redraw = true;
  return true;
}
...
bool Editor::fold_at_cursor()
{
  ...
  int hidden = Folding::hidden_line_count_for_header(buf.fold_ranges, header_line);
...
int Editor::buffer_line_for_visible_row(const FileBuffer &buf, int first_line, int row) const
{
