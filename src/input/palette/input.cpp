...
void Editor::handle_command_palette(int ch)
{
  ...
  auto apply_selected_completion = [&]()
  {
    ...
    std::string parse_body = body_start == std::string::npos ? "" : body.substr(body_start);
    ...
    const std::string chosen = command_palette_results[command_palette_selected].insert_text;
  ...
  { // PageUp
    ...
    if (!command_palette_results.empty())
    {
      command_palette_selected = std::max(0, command_palette_selected - 8);
      needs_redraw = true;
    }
  }
  else if (ch == 1016)
  { // PageDown
    ...
    {
      command_palette_selected =
          std::min((int)command_palette_results.size() - 1, command_palette_selected + 8);
      needs_redraw = true;
    }
  }
  else if (ch == 1012)
  ...
  { // End
    ...
    if (!command_palette_results.empty())
    {
      command_palette_selected = (int)command_palette_results.size() - 1;
      needs_redraw = true;
    }
  }
  else if (ch == '\n' || ch == 13)
  {
    if (trim_copy(command_palette_query).empty() && !command_palette_results.empty())
    {
      command_palette_query =
          command_palette_results[std::clamp(command_palette_selected,
                                             0,
