...
namespace
{
  ...
  const std::vector<CommandMeta> &command_palette_metadata()
  {
    static const std::vector<CommandMeta> meta = {
        ...
        {"discord", "Session", "Discord presence status", 70},
        ...
        {"declaration", "LSP", "Go to declaration", 70},
        {"lspdecl", "LSP", "Go to declaration", 66},
        {"lspdeclaration", "LSP", "Go to declaration", 64},
        {"typedefinition", "LSP", "Go to type definition", 68},
        {"lsptypedef", "LSP", "Go to type definition", 64},
        {"implementation", "LSP", "Go to implementation", 68},
        {"lspimpl", "LSP", "Go to implementation", 64},
        {"switchheader", "LSP", "Switch between header and source", 72},
        {"switchsourceheader", "LSP", "Switch between header and source", 68},
        {"lspheader", "LSP", "Switch between header and source", 66},
        {"jumpback", "Navigation", "Jump back in the jumplist", 72},
        ...
        {"deleteselection", "Selection", "Delete the selection (yank first)", 71},
        {"deleteobject", "Selection", "Delete inside/around a syntax object", 71},
        {"yankobject", "Selection", "Yank inside/around a syntax object", 70},
        {"yankselection", "Selection", "Yank the selection", 70},
        {"selectword", "Selection", "Select the word under the cursor", 73},
        {"selectline", "Selection", "Select the current line", 72},
        {"nextclass", "Navigation", "Go to the next class or type", 68},
        {"prevclass", "Navigation", "Go to the previous class or type", 67},
        {"keepprimary", "Selection", "Keep only the primary selection", 71},
        {"rotatecaret", "Selection", "Make the next selection the primary one", 69},
        {"addcaretbelow", "Selection", "Copy the selection onto the line below", 70},
        {"addcaretabove", "Selection", "Copy the selection onto the line above", 69},
        {"splitlines", "Selection", "One cursor per line of the selection", 70},
        {"selectoccurrences", "Selection", "Select every occurrence of the selection", 71},
        {"expand", "Selection", "Expand the selection to the enclosing syntax node", 74},
        {"shrink", "Selection", "Shrink the selection to a nested syntax node", 73},
        {"textobject", "Selection", "Select the inside/around of a syntax object", 72},
        {"nextfunction", "Navigation", "Go to the next function", 70},
        {"prevfunction", "Navigation", "Go to the previous function", 69},
        ...
        {"lazygit", "Git", "Open lazygit in the terminal", 72},
        {"gitpanel", "Git", "Open the git panel (2-5 views)", 76},
        {"gitcheckout", "Git", "Checkout branch or commit (-b creates)", 70},
        {"gitmerge", "Git", "Merge a branch into the current one", 70},
  ...
  std::vector<int> fuzzy_match_positions(const std::string &value, const std::string &query)
  {
...
void Editor::refresh_command_palette()
{
  ...
  const std::string seed =
      command_palette_theme_mode ? command_palette_theme_original : command_palette_query;
  auto add_result = [&](const std::string &insert_text,
                        std::string label,
                        std::string category,
                        std::string detail,
                        int score,
  ...
  if (cmd.empty()
      || (trimmed.find(' ') == std::string::npos && !(seed.size() > 0 && seed.back() == ' ')))
  {
    ...
    for (const auto &c : ex_commands())
    {
      ...
      if (needle.empty() || score >= 0)
      {
        ...
        add_result(c,
                   c,
                   meta ? meta->category : "Command",
                   meta ? meta->detail : "Run command",
                   score + (meta ? meta->priority : 40),
                   needle.empty() ? std::vector<int>{} : fuzzy_match_positions(c, needle));
      }
    }
    if (lua_api)
    {
      for (const auto &command : lua_api->commands())
      {
        ...
        if (needle.empty() || score >= 0)
        {
          add_result(command.name,
                     command.name,
                     "Plugin",
                     command.detail.empty() ? "Run plugin command" : command.detail,
                     score + 75,
                     needle.empty() ? std::vector<int>{}
                                    : fuzzy_match_positions(command.name, needle));
        }
      }
    }
  }
  else
  {
                       ...
                       int base_score = 100)
    {
      ...
      if (arg.empty() || score >= 0)
      {
        add_result(value,
                   ...
                   detail,
                   score + base_score,
                   arg.empty() ? std::vector<int>{} : fuzzy_match_positions(value, arg));
      }
    };
    ...
    else if (lcmd == "autosave")
    {
      const std::vector<std::string> opts = {
          "on", "off", "toggle", "status", "250", "500", "1000", "2000", "5000", "10000"};
    ...
    else if (lcmd == "debug" || lcmd == "debuggdb" || lcmd == "debuglldb")
    {
    ...
    else if (lcmd == "mkfile" || lcmd == "mkdir" || lcmd == "rm" || lcmd == "cppimpl"
             || lcmd == "cpppair")
    {
      auto paths = complete_workspace_path(arg);
      for (const auto &path : paths)
    ...
    else if (lcmd == "line" || lcmd == "goto")
    {
      ...
      std::vector<std::string> opts = {std::to_string(cur_line),
    ...
    else if (lcmd == "pluginpanel" && lua_api)
    {
      for (const auto &panel : lua_api->panels())
      {
        add_arg(panel.name,
                "Plugin Panel",
                panel.title.empty() ? "Show plugin panel" : panel.title,
                120);
      }
    }
    else if (lcmd == "e" || lcmd == "edit" || lcmd == "open" || lcmd == "w" || lcmd == "write"
             || lcmd == "wq" || lcmd == "x" || lcmd == "xit")
    {
      auto paths = complete_workspace_path(arg);
      for (const auto &path : paths)
      {
  ...
  std::sort(command_palette_results.begin(),
            command_palette_results.end(),
            [](const CommandPaletteSuggestion &a, const CommandPaletteSuggestion &b)
            {
              if (a.score != b.score)
              {
