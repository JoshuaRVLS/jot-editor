...
TreeSitterManager &manager(lua_State *L) { return api(L).tree_sitter_manager(); }

void check_main_thread(lua_State *L) {
  if (!api(L).is_main_thread()) luaL_error(L, "Tree-sitter API requires main thread");
}

TreeSitterHandle handle(lua_State *L, int index) {
  lua_Integer value = luaL_checkinteger(L, index);
  if (value <= 0) luaL_error(L, "invalid Tree-sitter handle");
...
int l_status(lua_State *L) {
  ...
  lua_newtable(L);
  lua_pushboolean(L, status.has_language); lua_setfield(L, -2, "has_language");
  lua_pushboolean(L, status.parser_loaded); lua_setfield(L, -2, "parser_loaded");
  lua_pushboolean(L, status.query_loaded); lua_setfield(L, -2, "query_loaded");
  lua_pushboolean(L, status.used_runtime_query); lua_setfield(L, -2, "used_runtime_query");
  lua_pushboolean(L, status.used_builtin_query); lua_setfield(L, -2, "used_builtin_query");
  lua_pushstring(L, status.language_id.c_str()); lua_setfield(L, -2, "language");
  lua_pushstring(L, status.parser_message.c_str()); lua_setfield(L, -2, "parser_message");
  lua_pushstring(L, status.query_message.c_str()); lua_setfield(L, -2, "query_message");
  return 1;
}
...
int l_parser(lua_State *L) {
  ...
  if (!h) return luaL_error(L, "unable to create parser"), 0;
  lua_pushinteger(L, static_cast<lua_Integer>(h));
  return 1;
}

int l_parser_delete(lua_State *L) { check_main_thread(L); lua_pushboolean(L, manager(L).delete_parser_handle(handle(L, 1))); return 1; }
...
int l_parse(lua_State *L) {
  ...
  if (!h) return luaL_error(L, "invalid parser or tree handle"), 0;
  lua_pushinteger(L, static_cast<lua_Integer>(h));
  return 1;
}

int l_tree_delete(lua_State *L) { check_main_thread(L); lua_pushboolean(L, manager(L).delete_tree_handle(handle(L, 1))); return 1; }

int l_query(lua_State *L) {
  check_main_thread(L);
  TreeSitterHandle h = manager(L).compile_query_handle(luaL_checkstring(L, 1), luaL_optstring(L, 2, ""));
  if (!h) return luaL_error(L, "query compilation failed"), 0;
  lua_pushinteger(L, static_cast<lua_Integer>(h));
  return 1;
}

int l_query_delete(lua_State *L) { check_main_thread(L); lua_pushboolean(L, manager(L).delete_query_handle(handle(L, 1))); return 1; }

int l_captures(lua_State *L) {
  check_main_thread(L);
  auto captures = manager(L).captures_for_handles(handle(L, 1), handle(L, 2),
  ...
  for (const auto &capture : captures) {
    lua_newtable(L); lua_pushstring(L, capture.name.c_str()); lua_setfield(L, -2, "name");
    ...
    lua_pushinteger(L, capture.start_row); lua_setfield(L, -2, "start_row");
    lua_pushinteger(L, capture.start_column); lua_setfield(L, -2, "start_column");
    lua_pushinteger(L, capture.end_row); lua_setfield(L, -2, "end_row");
    lua_pushinteger(L, capture.end_column); lua_setfield(L, -2, "end_column");
    lua_rawseti(L, -2, index++);
  }
  return 1;
}

int l_set_query(lua_State *L) { check_main_thread(L); std::string error; bool ok = manager(L).set_query_source(luaL_checkstring(L, 1), luaL_checkstring(L, 2), error); if (!ok) return luaL_error(L, "%s", error.c_str()), 0; lua_pushboolean(L, 1); return 1; }
int l_set_capture_color(lua_State *L) { check_main_thread(L); tree_sitter_set_capture_color(luaL_checkstring(L, 1), (int)luaL_checkinteger(L, 2)); return 0; }
int l_reload(lua_State *L) { check_main_thread(L); manager(L).reload(); return 0; }

void bind(lua_State *L, LuaAPI *a, const char *name, lua_CFunction fn) { lua_pushlightuserdata(L, a); lua_pushcclosure(L, fn, 1); lua_setfield(L, -2, name); }
}

TreeSitterManager &LuaAPI::tree_sitter_manager() { return editor->ts_manager_; }
bool LuaAPI::is_main_thread() const { return editor && editor->event_loop_.is_main_thread(); }
...
bool LuaAPI::load_treesitter_runtime(lua_State *L) {
  ...
  if (!fs::exists(path)) path = fs::path(JOT_LUA_SOURCE_DIR) / "treesitter" / "init.lua";
