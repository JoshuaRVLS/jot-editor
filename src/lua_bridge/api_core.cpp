...
LuaAPI &api(lua_State *L) {
  ...
  if (ud) return *static_cast<LuaAPI *>(ud);
  ...
  if (active_api) return *active_api;
...
std::pair<std::string, int> lua_capture_shell(const std::string &command) {
  ...
  while (fgets(buf.data(), (int)buf.size(), pipe) != nullptr) out += buf.data();
  int status = 0;
#ifdef _WIN32
  status = _pclose(pipe);
#else
...
int l_theme_apply(lua_State *L){lua_pushboolean(L,api(L).apply_theme_and_persist(luaL_checkstring(L,1)));return 1;}
...
int l_job_capture(lua_State *L){auto &a=api(L);const char*cmd=luaL_checkstring(L,1);std::string cwd;int cb=2;if(!lua_isfunction(L,2)){cwd=luaL_optstring(L,2,"");cb=3;}luaL_checktype(L,cb,LUA_TFUNCTION);lua_pushvalue(L,cb);int ref=luaL_ref(L,LUA_REGISTRYINDEX);std::string id="job."+std::to_string(ref);a.lua_callbacks[id]=ref;if(!a.run_job_capture(cmd,cwd,id)){a.lua_callbacks.erase(id);luaL_unref(L,LUA_REGISTRYINDEX,ref);lua_pushboolean(L,0);return 1;}return 0;}
int l_diagnostics_get(lua_State *L){api(L).push_diagnostics(L);return 1;}
int l_buf_set_var(lua_State *L){api(L).set_buffer_var(L);return 0;}
int l_buf_get_var(lua_State *L){api(L).push_buffer_var(L);return 1;}
int l_buf_del_var(lua_State *L){api(L).delete_buffer_var(L);return 0;}
int l_mark_set(lua_State *L){api(L).set_mark(L);return 1;}
int l_mark_get(lua_State *L){api(L).push_mark(L);return 1;}
int l_mark_jump(lua_State *L){api(L).jump_mark(L);return 1;}
int l_mark_del(lua_State *L){api(L).delete_mark(L);return 1;}
int l_mark_list(lua_State *L){api(L).push_mark_list(L);return 1;}
int l_status_register(lua_State *L){api(L).register_status_segment(L);return 0;}
int l_status_unregister(lua_State *L){api(L).unregister_status_segment(L);return 0;}
int l_config_get(lua_State *L){api(L).lua_config_get(L,0);return 1;}
int l_config_get_number(lua_State *L){api(L).lua_config_get(L,1);return 1;}
int l_config_get_bool(lua_State *L){api(L).lua_config_get(L,2);return 1;}
int l_config_set(lua_State *L){api(L).config_set_from_lua(L);return 0;}
int l_config_unset(lua_State *L){api(L).config_unset_from_lua(L);return 0;}
int l_config_has(lua_State *L){api(L).config_has_from_lua(L);return 1;}
int l_config_keys(lua_State *L){api(L).push_config_keys(L);return 1;}
int l_config_path(lua_State *L){api(L).push_config_path(L);return 1;}
int l_editor_info(lua_State *L){api(L).push_editor_info(L);return 1;}
int l_theme_current(lua_State *L){api(L).push_theme_current(L);return 1;}
int l_git_info(lua_State *L){api(L).push_git_info(L);return 1;}
int l_git_status(lua_State *L){api(L).push_git_status(L);return 1;}
int l_git_stage(lua_State *L){api(L).git_stage_from_lua(L);return 1;}
int l_git_unstage(lua_State *L){api(L).git_unstage_from_lua(L);return 1;}
int l_git_stage_all(lua_State *L){api(L).git_stage_all_from_lua(L);return 1;}
int l_git_unstage_all(lua_State *L){api(L).git_unstage_all_from_lua(L);return 1;}
int l_git_commit(lua_State *L){api(L).git_commit_from_lua(L);return 1;}
int l_git_refresh(lua_State *L){api(L).git_refresh_from_lua(L);return 0;}
int l_task_list(lua_State *L){api(L).push_task_list(L);return 1;}
int l_task_run(lua_State *L){api(L).run_task_from_lua(L);return 1;}
int l_task_rerun(lua_State *L){api(L).rerun_task_from_lua(L);return 1;}
int l_symbols_list(lua_State *L){api(L).push_symbols(L);return 1;}
int l_dbg_configs(lua_State *L){api(L).push_debugger_configs(L);return 1;}
int l_dbg_run_config(lua_State *L){api(L).run_debugger_config_from_lua(L);return 1;}
int l_buf_current(lua_State *L){api(L).push_buffer_current(L);return 1;}
int l_buf_count(lua_State *L){api(L).push_buffer_count(L);return 1;}
int l_buf_text(lua_State *L){api(L).push_buffer_text(L);return 1;}
int l_buf_meta(lua_State *L){api(L).push_buffer_meta(L);return 1;}
int l_buf_selection(lua_State *L){api(L).push_buffer_selection(L);return 1;}
int l_buf_bookmarks(lua_State *L){api(L).push_buffer_bookmarks(L);return 1;}
int l_buf_folds(lua_State *L){api(L).push_buffer_folds(L);return 1;}
int l_clip_copy(lua_State *L){api(L).clipboard_copy_from_lua(L);return 0;}
int l_clip_cut(lua_State *L){api(L).clipboard_cut_from_lua(L);return 0;}
int l_clip_paste(lua_State *L){api(L).clipboard_paste_from_lua(L);return 0;}
int l_terminal_list(lua_State *L){api(L).push_terminal_list(L);return 1;}
int l_terminal_write(lua_State *L){api(L).terminal_write_from_lua(L);return 1;}
int l_terminal_close(lua_State *L){api(L).terminal_close_from_lua(L);return 1;}
int l_terminal_activate(lua_State *L){api(L).terminal_activate_from_lua(L);return 1;}
int l_terminal_spawn(lua_State *L){api(L).terminal_spawn_from_lua(L);return 1;}
int l_workspace_path(lua_State *L){api(L).push_workspace_path(L);return 1;}
int l_recent_files(lua_State *L){api(L).push_recent_files(L);return 1;}
int l_recent_workspaces(lua_State *L){api(L).push_recent_workspaces(L);return 1;}
int l_lsp_clients(lua_State *L){api(L).push_lsp_clients(L);return 1;}
int l_popup(lua_State *L){api(L).popup_from_lua(L);return 0;}
int l_ui_command_palette(lua_State *L){api(L).host().io.open_command_palette(luaL_optstring(L,1,""));return 0;}
int l_search_info(lua_State *L){api(L).push_search_info(L);return 1;}
int l_search_matches(lua_State *L){api(L).push_search_matches(L);return 1;}
int l_picker_active(lua_State *L){api(L).push_picker_active(L);return 1;}
int l_picker_info(lua_State *L){api(L).push_picker_info(L);return 1;}
int l_picker_items(lua_State *L){api(L).push_picker_items(L);return 1;}
int l_picker_accept(lua_State *L){api(L).picker_accept_from_lua(L);return 1;}
int l_picker_close(lua_State *L){api(L).picker_close_from_lua(L);return 1;}
int l_buf_tokens(lua_State *L){api(L).push_buffer_tokens(L);return 1;}
int l_lsp_request_hover(lua_State *L){api(L).lsp_request_from_lua(L,0);return 0;}
int l_lsp_request_definition(lua_State *L){api(L).lsp_request_from_lua(L,1);return 0;}
int l_lsp_request_symbols(lua_State *L){api(L).lsp_request_from_lua(L,2);return 0;}
int l_lsp_diagnostics(lua_State *L){api(L).push_lsp_diagnostics(L);return 1;}
int l_lsp_results(lua_State *L){api(L).push_lsp_last_results(L);return 1;}
int l_lsp_completions(lua_State *L){api(L).push_lsp_completions(L);return 1;}
int l_lsp_request_completion(lua_State *L){api(L).lsp_request_from_lua(L,3);return 0;}
int l_events_subscribe(lua_State *L){api(L).events_subscribe_from_lua(L);return 1;}
int l_events_unsubscribe(lua_State *L){api(L).events_unsubscribe_from_lua(L);return 0;}
int l_viewport_info(lua_State *L){api(L).push_viewport_info(L);return 1;}
int l_viewport_line_at(lua_State *L){api(L).push_viewport_line_at(L);return 1;}
int l_viewport_scroll_top(lua_State *L){api(L).viewport_scroll_top_from_lua(L);return 0;}
int l_viewport_scroll_lines(lua_State *L){api(L).viewport_scroll_lines_from_lua(L);return 0;}
int l_viewport_scroll_col(lua_State *L){api(L).viewport_scroll_col_from_lua(L);return 0;}
int l_viewport_reveal(lua_State *L){api(L).viewport_reveal_from_lua(L);return 0;}
int l_filetree_root(lua_State *L){api(L).push_filetree_root(L);return 1;}
int l_filetree_tree(lua_State *L){api(L).push_filetree_tree(L);return 1;}
int l_filetree_children(lua_State *L){api(L).push_filetree_children(L);return 1;}
int l_buf_select(lua_State *L){api(L).buffer_select_from_lua(L);return 0;}
int l_buf_clear_selection(lua_State *L){api(L).buffer_clear_selection_from_lua(L);return 0;}
int l_buf_lines(lua_State *L){api(L).push_buffer_lines(L);return 1;}
int l_timer_set_timeout(lua_State *L){api(L).timer_set_from_lua(L,false);return 1;}
int l_timer_set_interval(lua_State *L){api(L).timer_set_from_lua(L,true);return 1;}
int l_timer_clear(lua_State *L){api(L).timer_clear_from_lua(L);return 0;}
int l_dbg_state(lua_State *L){api(L).push_debugger_state(L);return 1;}
int l_dbg_breakpoints(lua_State *L){api(L).push_debugger_breakpoints(L);return 1;}
int l_dbg_toggle_bp(lua_State *L){api(L).debugger_toggle_breakpoint_from_lua(L);return 1;}
int l_dbg_has_bp(lua_State *L){api(L).debugger_has_breakpoint_from_lua(L);return 1;}
int l_dbg_request_stack(lua_State *L){api(L).debugger_request_from_lua(L,0);return 0;}
int l_dbg_request_variables(lua_State *L){api(L).debugger_request_from_lua(L,1);return 0;}
int l_dbg_request_threads(lua_State *L){api(L).debugger_request_from_lua(L,2);return 0;}
int l_theme_palette(lua_State *L){api(L).push_theme_palette(L);return 1;}
int l_clip_get(lua_State *L){api(L).push_clipboard_text(L);return 1;}
int l_motion_word_next(lua_State *L){api(L).motion_from_lua(L,0);return 0;}
int l_motion_word_prev(lua_State *L){api(L).motion_from_lua(L,1);return 0;}
int l_motion_line_start(lua_State *L){api(L).motion_from_lua(L,2);return 0;}
int l_motion_line_end(lua_State *L){api(L).motion_from_lua(L,3);return 0;}
int l_motion_file_start(lua_State *L){api(L).motion_from_lua(L,4);return 0;}
int l_motion_file_end(lua_State *L){api(L).motion_from_lua(L,5);return 0;}
int l_motion_matching_bracket(lua_State *L){api(L).motion_from_lua(L,6);return 0;}
int l_motion_select_function(lua_State *L){api(L).motion_from_lua(L,7);return 0;}
int l_sidebar_info(lua_State *L){api(L).push_sidebar_info(L);return 1;}
int l_sidebar_set_view(lua_State *L){api(L).sidebar_set_view_from_lua(L);return 0;}
int l_lsp_disabled(lua_State *L){api(L).push_lsp_disabled(L);return 1;}
int l_lsp_set_enabled(lua_State *L){api(L).lsp_set_enabled_from_lua(L);return 0;}
int l_lsp_install(lua_State *L){api(L).lsp_install_from_lua(L);return 1;}
int l_lsp_remove(lua_State *L){api(L).lsp_remove_from_lua(L);return 1;}
int l_lsp_restart_all(lua_State *L){api(L).lsp_restart_all_from_lua(L);return 0;}
int l_buf_filetype(lua_State *L){api(L).push_buffer_filetype(L);return 1;}
int l_buf_get_line(lua_State *L){api(L).push_buffer_get_line(L);return 1;}
int l_git_diff(lua_State *L){api(L).git_diff_from_lua(L);return 1;}
...
bool LuaAPI::init(){
 ...
 inject(L,this,"get_current_buffer",l_get_buffer);inject(L,this,"set_current_buffer",l_set_buffer);inject(L,this,"get_selection",l_get_selection);inject(L,this,"replace_selection",l_replace);inject(L,this,"insert_text",l_insert);inject(L,this,"cursor",l_cursor);inject(L,this,"set_cursor",l_set_cursor);inject(L,this,"current_file",l_current_file);inject(L,this,"open_file",l_open);inject(L,this,"save",l_save);inject(L,this,"execute",l_execute);inject(L,this,"run_job",l_job);inject(L,this,"show_picker",l_picker);inject(L,this,"show_panel",l_panel_show);inject(L,this,"register_keymap",l_register_keymap);inject(L,this,"register_panel",l_register_panel);
  ...
  lua_newtable(L);field(L,"get_text",l_get_buffer);field(L,"set_text",l_set_buffer);field(L,"get_selection",l_get_selection);field(L,"replace_selection",l_replace);field(L,"insert",l_insert);field(L,"cursor",l_cursor);field(L,"set_cursor",l_set_cursor);field(L,"list",l_buffer_list);field(L,"current",l_buf_current);field(L,"count",l_buf_count);field(L,"text",l_buf_text);field(L,"meta",l_buf_meta);field(L,"selection",l_buf_selection);field(L,"bookmarks",l_buf_bookmarks);field(L,"folds",l_buf_folds);field(L,"tokens",l_buf_tokens);field(L,"select",l_buf_select);field(L,"clear_selection",l_buf_clear_selection);field(L,"lines",l_buf_lines);field(L,"filetype",l_buf_filetype);field(L,"get_line",l_buf_get_line);field(L,"switch",l_switch_buffer);field(L,"set_var",l_buf_set_var);field(L,"get_var",l_buf_get_var);field(L,"del_var",l_buf_del_var);lua_setfield(L,-2,"buffer");
  ...
  lua_newtable(L);field(L,"register",l_register_keymap);lua_setfield(L,-2,"keymap");lua_newtable(L);field(L,"run",l_job);field(L,"capture",l_job_capture);lua_setfield(L,-2,"job");lua_newtable(L);field(L,"set",[](lua_State*s){auto&a=api(s);luaL_checktype(s,2,LUA_TTABLE);lua_getfield(s,2,"fg");int fg=lua_isnumber(s,-1)?(int)lua_tointeger(s,-1):-1;lua_pop(s,1);lua_getfield(s,2,"bg");int bg=lua_isnumber(s,-1)?(int)lua_tointeger(s,-1):-1;lua_pop(s,1);a.set_theme_color(luaL_optstring(s,1,""),fg,bg);return 0;});lua_setfield(L,-2,"theme");
  ...
  field(L, "list", l_buffer_list); field(L, "switch", l_switch_buffer);
  field(L, "current", l_buf_current); field(L, "count", l_buf_count);
  field(L, "text", l_buf_text); field(L, "meta", l_buf_meta);
  field(L, "selection", l_buf_selection);
  field(L, "bookmarks", l_buf_bookmarks); field(L, "folds", l_buf_folds);
  field(L, "tokens", l_buf_tokens); field(L, "select", l_buf_select);
  field(L, "clear_selection", l_buf_clear_selection);
  field(L, "lines", l_buf_lines); field(L, "filetype", l_buf_filetype);
  field(L, "get_line", l_buf_get_line);
  field(L, "set_var", l_buf_set_var); field(L, "get_var", l_buf_get_var);
  ...
  field(L, "popup", l_popup); field(L, "command_palette", l_ui_command_palette);
  lua_pop(L, 2);
  lua_newtable(L); field(L, "execute", l_editor_execute); field(L, "request_redraw", l_editor_redraw);
  field(L, "info", l_editor_info); lua_setfield(L, -2, "editor");
  lua_newtable(L); field(L, "execute", l_command); field(L, "info", l_search_info); field(L, "matches", l_search_matches); command_field(L, this, "next", ":next"); command_field(L, this, "previous", ":prev"); lua_setfield(L, -2, "search");
  // jot.symbols: native SymbolIndex extraction over the live buffer.
  lua_newtable(L); field(L, "list", l_symbols_list);
  ...
  lua_newtable(L); field(L, "execute", l_command); field(L, "open", l_open_workspace); field(L, "path", l_workspace_path); field(L, "recent", l_recent_workspaces); lua_setfield(L, -2, "workspace");
  lua_newtable(L); field(L, "execute", l_command); field(L, "list", l_terminal_list); field(L, "spawn", l_terminal_spawn); field(L, "close", l_terminal_close); field(L, "activate", l_terminal_activate); field(L, "write", l_terminal_write); command_field(L, this, "toggle", "Toggle Terminal"); command_field(L, this, "new", "New Terminal"); lua_setfield(L, -2, "terminal");
  lua_newtable(L); field(L, "execute", l_command); field(L, "list", l_task_list); field(L, "run", l_task_run); field(L, "rerun", l_task_rerun); command_field(L, this, "show", "Tasks"); lua_setfield(L, -2, "tasks");
  lua_newtable(L); field(L, "execute", l_command); field(L, "clients", l_lsp_clients); field(L, "diagnostics", l_lsp_diagnostics); field(L, "results", l_lsp_results); field(L, "completions", l_lsp_completions); field(L, "request_hover", l_lsp_request_hover); field(L, "request_definition", l_lsp_request_definition); field(L, "request_symbols", l_lsp_request_symbols); field(L, "request_completion", l_lsp_request_completion); field(L, "disabled", l_lsp_disabled); field(L, "set_enabled", l_lsp_set_enabled); field(L, "install", l_lsp_install); field(L, "remove", l_lsp_remove); field(L, "restart_all", l_lsp_restart_all); command_field(L, this, "definition", "LSP Definition"); command_field(L, this, "back", "LSP Back"); command_field(L, this, "completion", "LSP Completion"); lua_setfield(L, -2, "lsp");
  lua_newtable(L); field(L, "execute", l_command); field(L, "configs", l_dbg_configs); field(L, "run_config", l_dbg_run_config); field(L, "state", l_dbg_state); field(L, "breakpoints", l_dbg_breakpoints); field(L, "toggle_breakpoint", l_dbg_toggle_bp); field(L, "has_breakpoint", l_dbg_has_bp); field(L, "request_stack", l_dbg_request_stack); field(L, "request_variables", l_dbg_request_variables); field(L, "request_threads", l_dbg_request_threads); command_field(L, this, "continue", "Debug Continue"); command_field(L, this, "pause", "Debug Pause"); command_field(L, this, "step_in", "Debug Step In"); command_field(L, this, "step_over", "Debug Step Over"); command_field(L, this, "step_out", "Debug Step Out"); command_field(L, this, "stop", "Debug Stop"); lua_setfield(L, -2, "debugger");
  lua_newtable(L); field(L, "execute", l_command); field(L, "info", l_git_info); field(L, "status", l_git_status); field(L, "stage", l_git_stage); field(L, "unstage", l_git_unstage); field(L, "stage_all", l_git_stage_all); field(L, "unstage_all", l_git_unstage_all); field(L, "commit", l_git_commit); field(L, "refresh", l_git_refresh); field(L, "diff", l_git_diff); lua_setfield(L, -2, "git");
  ...
  lua_newtable(L); field(L, "get", l_config_get); field(L, "get_number", l_config_get_number); field(L, "get_bool", l_config_get_bool); field(L, "set", l_config_set); field(L, "unset", l_config_unset); field(L, "has", l_config_has); field(L, "keys", l_config_keys); field(L, "path", l_config_path); lua_setfield(L, -2, "config");
  lua_newtable(L); field(L, "get", l_diagnostics_get);
  lua_setfield(L, -2, "diagnostics");
  lua_newtable(L); field(L, "set", l_mark_set); field(L, "get", l_mark_get);
  field(L, "jump", l_mark_jump); field(L, "del", l_mark_del);
  field(L, "list", l_mark_list);
  lua_setfield(L, -2, "marks");
  lua_newtable(L); field(L, "copy", l_clip_copy); field(L, "cut", l_clip_cut); field(L, "paste", l_clip_paste); field(L, "get", l_clip_get); lua_setfield(L, -2, "clipboard");
  lua_newtable(L); field(L, "active", l_picker_active); field(L, "info", l_picker_info); field(L, "items", l_picker_items); field(L, "accept", l_picker_accept); field(L, "close", l_picker_close); lua_setfield(L, -2, "picker");
  lua_newtable(L); field(L, "subscribe", l_events_subscribe); field(L, "unsubscribe", l_events_unsubscribe); lua_setfield(L, -2, "events");
  lua_newtable(L); field(L, "set_timeout", l_timer_set_timeout); field(L, "set_interval", l_timer_set_interval); field(L, "clear", l_timer_clear); lua_setfield(L, -2, "timer");
  lua_newtable(L); field(L, "word_next", l_motion_word_next); field(L, "word_prev", l_motion_word_prev); field(L, "line_start", l_motion_line_start); field(L, "line_end", l_motion_line_end); field(L, "file_start", l_motion_file_start); field(L, "file_end", l_motion_file_end); field(L, "matching_bracket", l_motion_matching_bracket); field(L, "select_function", l_motion_select_function); lua_setfield(L, -2, "motion");
  lua_newtable(L); field(L, "info", l_sidebar_info); field(L, "set_view", l_sidebar_set_view); lua_setfield(L, -2, "sidebar");
  lua_newtable(L); field(L, "info", l_viewport_info); field(L, "line_at", l_viewport_line_at); field(L, "scroll_top", l_viewport_scroll_top); field(L, "scroll_lines", l_viewport_scroll_lines); field(L, "scroll_col", l_viewport_scroll_col); field(L, "reveal", l_viewport_reveal); lua_setfield(L, -2, "viewport");
  lua_newtable(L); field(L, "root", l_filetree_root); field(L, "tree", l_filetree_tree); field(L, "children", l_filetree_children); lua_setfield(L, -2, "filetree");
  lua_newtable(L); field(L, "register", l_status_register);
  ...
  lua_setglobal(L, "jot");
   ...
   lua_getglobal(L,"jot");lua_getfield(L,-1,"theme");field(L,"set_color",[](lua_State*s){auto&a=api(s);luaL_checktype(s,2,LUA_TTABLE);lua_getfield(s,2,"fg");int fg=lua_isnumber(s,-1)?(int)lua_tointeger(s,-1):-1;lua_pop(s,1);lua_getfield(s,2,"bg");int bg=lua_isnumber(s,-1)?(int)lua_tointeger(s,-1):-1;lua_pop(s,1);a.set_theme_color(luaL_optstring(s,1,""),fg,bg);return 0;});field(L,"palette",l_theme_palette);lua_pop(L,2);
  lua_getglobal(L,"jot");lua_getfield(L,-1,"ui");field(L,"register_panel",l_register_panel);lua_pop(L,2);
   load_treesitter_runtime(L);
   load_plugins();
  return true;
}
 void LuaAPI::cleanup(){if(!lua_state)return;cancel_all_timers();clear_floats();lua_close(static_cast<lua_State*>(lua_state));lua_state=nullptr;lua_callbacks.clear();lua_initialized=false;timer_entries_.clear();}
 void LuaAPI::clear_runtime_state(){plugin_commands.clear();plugin_keymaps.clear();plugin_autocmds.clear();plugin_panels.clear();status_segments_.clear();plugin_load_status.clear();cancel_all_timers();clear_floats();if(lua_state){for(auto&x:lua_callbacks)luaL_unref(static_cast<lua_State*>(lua_state),LUA_REGISTRYINDEX,x.second);for(auto&kv:event_subscribers_)for(auto&s:kv.second)luaL_unref(static_cast<lua_State*>(lua_state),LUA_REGISTRYINDEX,s.ref);}lua_callbacks.clear();event_subscribers_.clear();pending_lsp_hover.clear();pending_lsp_definition.clear();pending_lsp_symbols.clear();pending_lsp_completion.clear();pending_debugger_stack.clear();pending_debugger_variables.clear();pending_debugger_threads.clear();pending_debugger_session=-1;edit_snapshots_.clear();last_edit_=LuaEditDelta{};}
...
bool LuaAPI::call_callback_string(const std::string &id,const std::string &arg){if(!lua_initialized||!editor||!editor->event_loop_.is_main_thread())return false;auto it=lua_callbacks.find(id);if(it==lua_callbacks.end())return false;lua_State*L=static_cast<lua_State*>(lua_state);int top=lua_gettop(L);lua_rawgeti(L,LUA_REGISTRYINDEX,it->second);lua_pushstring(L,arg.c_str());bool ok=lua_pcall(L,1,0,0)==LUA_OK;if(!ok){std::cerr<<"Lua callback error: "<<lua_tostring(L,-1)<<"\n";}lua_settop(L,top);return ok;}
 bool LuaAPI::call_callback_event(const std::string &id,const std::string &event,const std::string &filepath,int buffer){if(!lua_initialized||!editor||!editor->event_loop_.is_main_thread())return false;auto it=lua_callbacks.find(id);if(it==lua_callbacks.end())return false;lua_State*L=static_cast<lua_State*>(lua_state);int top=lua_gettop(L);lua_rawgeti(L,LUA_REGISTRYINDEX,it->second);lua_newtable(L);lua_pushstring(L,event.c_str());lua_setfield(L,-2,"event");lua_pushstring(L,filepath.c_str());lua_setfield(L,-2,"path");if(buffer<0)lua_pushnil(L);else lua_pushinteger(L,buffer+1);lua_setfield(L,-2,"buffer");if(buffer>=0&&buffer<(int)editor->buffers.size()){const Cursor pos=editor->buffers[(size_t)buffer].cursor;lua_pushinteger(L,pos.y+1);lua_setfield(L,-2,"line");lua_pushinteger(L,pos.x+1);lua_setfield(L,-2,"column");}if(event=="BufChange"&&last_edit_.valid){lua_pushinteger(L,last_edit_.start_line);lua_setfield(L,-2,"edit_start_line");lua_pushinteger(L,last_edit_.start_col);lua_setfield(L,-2,"edit_start_col");lua_pushinteger(L,last_edit_.end_line);lua_setfield(L,-2,"edit_end_line");lua_pushinteger(L,last_edit_.end_col);lua_setfield(L,-2,"edit_end_col");lua_pushstring(L,last_edit_.inserted.c_str());lua_setfield(L,-2,"edit_inserted");lua_pushstring(L,last_edit_.removed.c_str());lua_setfield(L,-2,"edit_removed");lua_pushboolean(L,last_edit_.multiline);lua_setfield(L,-2,"edit_multiline");}if(lua_pcall(L,1,0,0)!=LUA_OK){std::cerr<<"Lua event callback error: "<<lua_tostring(L,-1)<<"\n";lua_settop(L,top);return false;}lua_settop(L,top);return true;}
void LuaAPI::on_buffer_open(const std::string&f){if(editor)editor->notify_lsp_open(f);fire_autocmd("BufOpen",f,-1);emit_buffer_event("buffer.open",f);}
...
static std::string lua_join_edit_range(const std::vector<std::string> &lines,
  ...
  std::string out;
  if (first > last || first >= lines.size()) return out;
  for (size_t i = first; i <= last && i < lines.size(); i++) {
    const std::string &line = lines[i];
    size_t from = 0;
    size_t to = line.size();
    if (i == first) from = std::min(cut_left, line.size());
    if (i == last) {
      to = std::min(end_col, line.size());
      if (i == first && to < from) to = from;
    }
    if (i > first) out += '\n';
    if (to > from) out.append(line, from, to - from);
  }
  return out;
}
...
void LuaAPI::on_buffer_change(const std::string &f, const std::string &) {
  ...
  if (editor) {
    for (size_t i = 0; i < editor->buffers.size(); i++) {
      if (editor->buffers[i].filepath == f) { index = (int)i; break; }
    }
    if (index < 0 && !f.empty()) {
      std::error_code ec;
  ...
  if (index >= 0 && index < (int)editor->buffers.size()) {
    ...
    for (const auto &x : plugin_autocmds) {
      if (x.event == "BufChange") { has_listener = true; break; }
    }
    if (has_listener) {
...
void LuaAPI::on_buffer_save(const std::string&f){if(editor)editor->notify_lsp_save(f);fire_autocmd("BufSave",f,-1);emit_buffer_event("buffer.save",f);}
...
bool LuaAPI::run_job_capture(const std::string &command, const std::string &cwd,
  ...
  if (!editor || !editor->task_queue_ || command.empty()) return false;
  std::string full = command;
  if (!cwd.empty()) full = "cd " + lua_shell_quote(cwd) + " && " + command;
...
bool LuaAPI::deliver_job_result(const std::string &callback,
  ...
  lua_newtable(L);
  lua_pushstring(L, output.c_str()); lua_setfield(L, -2, "output");
  lua_pushinteger(L, exit_code); lua_setfield(L, -2, "exit_code");
...
int LuaAPI::resolve_buffer_arg(lua_State *L, int arg_index) {
  if (!editor || editor->buffers.empty()) return -1;
  int idx = -1;
  if (arg_index <= lua_gettop(L) && !lua_isnil(L, arg_index)) {
    if (lua_isnumber(L, arg_index)) {
      ...
      for (size_t i = 0; i < editor->buffers.size(); i++) {
        if (editor->buffers[i].filepath == path) { idx = (int)i; break; }
      }
    }
  } else {
    idx = editor->current_buffer;
  }
  if (idx < 0 || idx >= (int)editor->buffers.size()) return -1;
  return idx;
}

void LuaAPI::push_diagnostics(lua_State *L) {
  int idx = resolve_buffer_arg(L, 1);
  if (idx < 0) { lua_pushnil(L); return; }
  FileBuffer &buf = editor->buffers[(size_t)idx];
  lua_newtable(L);
  int n = 1;
  for (const auto &d : buf.diagnostics) {
    lua_newtable(L);
    lua_pushinteger(L, d.line + 1); lua_setfield(L, -2, "line");
    lua_pushinteger(L, d.col + 1); lua_setfield(L, -2, "col");
    lua_pushinteger(L, d.end_line + 1); lua_setfield(L, -2, "end_line");
    lua_pushinteger(L, d.end_col + 1); lua_setfield(L, -2, "end_col");
    lua_pushinteger(L, d.severity); lua_setfield(L, -2, "severity");
    lua_pushstring(L, lua_diag_severity_name(d.severity));
    lua_setfield(L, -2, "severity_name");
    lua_pushstring(L, d.message.c_str()); lua_setfield(L, -2, "message");
    lua_rawseti(L, -2, n++);
  }
}
...
void LuaAPI::set_mark(lua_State *L) {
  ...
  if (std::isupper((unsigned char)c)) {
    if (buf.filepath.empty()) { lua_pushboolean(L, 0); return; }
    GlobalMark gm;
...
void LuaAPI::push_mark(lua_State *L) {
  ...
  if (std::islower((unsigned char)c)) {
    ...
    if (it == buf.marks.end()) { lua_pushnil(L); return; }
    lua_newtable(L);
    lua_pushinteger(L, editor->current_buffer + 1);
    lua_setfield(L, -2, "buffer");
    lua_pushstring(L, buf.filepath.c_str()); lua_setfield(L, -2, "path");
    lua_pushinteger(L, it->second.y + 1); lua_setfield(L, -2, "line");
    lua_pushinteger(L, it->second.x + 1); lua_setfield(L, -2, "col");
    lua_pushboolean(L, 0); lua_setfield(L, -2, "global");
    return;
  }
  if (std::isupper((unsigned char)c) && editor) {
    auto it = editor->global_marks.find(c);
    if (it == editor->global_marks.end()) { lua_pushnil(L); return; }
    lua_newtable(L);
    lua_pushstring(L, it->second.filepath.c_str());
    lua_setfield(L, -2, "path");
    lua_pushinteger(L, it->second.line + 1); lua_setfield(L, -2, "line");
    lua_pushinteger(L, it->second.col + 1); lua_setfield(L, -2, "col");
    lua_pushboolean(L, 1); lua_setfield(L, -2, "global");
    return;
  }
  lua_pushnil(L);
}
...
void LuaAPI::jump_mark(lua_State *L) {
  ...
  char c = s[0];
  if (!editor) { lua_pushboolean(L, 0); return; }
  ...
  int idx = -1;
  for (size_t i = 0; i < editor->buffers.size(); i++) {
    if (editor->buffers[i].filepath == target_path) { idx = (int)i; break; }
  }
  if (idx < 0) {
    if (target_path.empty()) { lua_pushboolean(L, 0); return; }
    editor->open_file(target_path);
    for (size_t i = 0; i < editor->buffers.size(); i++) {
      if (editor->buffers[i].filepath == target_path) { idx = (int)i; break; }
    }
  }
  if (idx < 0) { lua_pushboolean(L, 0); return; }
  if (editor->current_buffer != idx) {
    editor->host_api->core.switch_buffer(idx);
  }
  FileBuffer &buf = editor->buffers[(size_t)idx];
  if (buf.line_count() == 0) { lua_pushboolean(L, 0); return; }
...
void LuaAPI::push_mark_list(lua_State *L) {
  ...
  if (editor->current_buffer >= 0 &&
    ...
    for (const auto &kv : buf.marks) {
      lua_newtable(L);
      lua_pushlstring(L, &kv.first, 1); lua_setfield(L, -2, "name");
      lua_pushboolean(L, 0); lua_setfield(L, -2, "global");
      lua_pushinteger(L, editor->current_buffer + 1);
      lua_setfield(L, -2, "buffer");
      lua_pushstring(L, buf.filepath.c_str()); lua_setfield(L, -2, "path");
      lua_pushinteger(L, kv.second.y + 1); lua_setfield(L, -2, "line");
      lua_pushinteger(L, kv.second.x + 1); lua_setfield(L, -2, "col");
      lua_rawseti(L, -2, n++);
    }
  }
  for (const auto &kv : editor->global_marks) {
    ...
    lua_setfield(L, -2, "path");
    lua_pushinteger(L, kv.second.line + 1); lua_setfield(L, -2, "line");
    lua_pushinteger(L, kv.second.col + 1); lua_setfield(L, -2, "col");
    lua_rawseti(L, -2, n++);
  }
}
...
std::vector<RenderedStatusSegment> LuaAPI::render_status_segments() {
  ...
  for (const auto &seg : status_segments_) {
    ...
    if (it == lua_callbacks.end()) continue;
    ...
    if (lua_isnumber(L, -1)) {
      ...
      if (f >= 0 && f <= 255) rs.fg = f;
    }
    lua_settop(L, top);
    if (!rs.text.empty()) out.push_back(std::move(rs));
  }
  lua_settop(L, top);
  return out;
}
...
void LuaAPI::push_editor_info(lua_State *L) {
  ...
  if (has_buffer) {
    ...
    if (buf.cursor.y >= 0 && buf.cursor.y < (int)buf.line_count()) {
      ...
      int start = x;
      while (start > 0 && is_word_char(text[(size_t)start - 1])) start--;
      int end = x;
      while (end < (int)text.size() && is_word_char(text[(size_t)end])) end++;
      if (start < end) {
...
void LuaAPI::git_refresh_from_lua(lua_State *L) {
  if (editor) editor->refresh_git_status(true);
}
...
void LuaAPI::push_symbols(lua_State *L) {
  ...
  if (idx < 0) return;
  FileBuffer &buf = editor->buffers[(size_t)idx];
  if (buf.is_lazy() || buf.filepath.empty() || buf.line_count() == 0) return;
...
static int lua_syntax_fg(const Theme &theme, int token) {
  switch (token) {
  case 1: return theme.fg_keyword;
  case 2: return theme.fg_string;
  case 3: return theme.fg_comment;
  case 4: return theme.fg_number;
  case 5: return theme.fg_type;
  case 6: return theme.fg_function;
  case 7: return theme.fg_variable;
  case 8: return theme.fg_constant;
  case 9: return theme.fg_builtin;
  case 10: return theme.fg_operator;
  case 11: return theme.fg_punctuation;
  case 12: return theme.fg_tag;
  case 13: return theme.fg_attribute;
  case 14: return theme.fg_namespace;
  case 15: return theme.fg_module;
  case 16: return theme.fg_parameter;
  case 17: return theme.fg_field;
  case 18: return theme.fg_keyword_control;
  case 19: return theme.fg_keyword_storage;
  case 20: return theme.fg_keyword_preproc;
  case 21: return theme.fg_function_method;
  case 22: return theme.fg_function_constructor;
  case 23: return theme.fg_type_builtin;
  case 24: return theme.fg_constant_macro;
  case 25: return theme.fg_string_escape;
  case 26: return theme.fg_punctuation_bracket;
  case 27: return theme.fg_punctuation_delimiter;
  default: return theme.fg_command;
  }
}
...
void LuaAPI::push_search_info(lua_State *L) {
  ...
  lua_push_int_field(
      L, "result_index",
      editor->search_result_index >= 0 ?
          (long long)editor->search_result_index + 1 : 0);
}
...
void LuaAPI::push_picker_info(lua_State *L) {
  lua_newtable(L);
  if (!editor || !editor->show_quick_pick) return;
...
void LuaAPI::push_picker_items(lua_State *L) {
  ...
  for (const QuickPickItem &item : editor->quick_pick_items) {
    ...
    lua_push_int_field(L, "line", item.line >= 0 ? (long long)item.line + 1
                                                 : 0);
    lua_push_int_field(L, "column", item.col >= 0 ? (long long)item.col + 1
                                                  : 0);
...
static bool lua_deliver_one_shot(const std::string &id,
  ...
  if (id.empty() || !lua_state) return false;
...
bool LuaAPI::try_deliver_lsp_hover(const LSPHoverResult &hover) {
  if (pending_lsp_hover.empty()) return false;
  if (!editor || editor->buffers.empty() || editor->current_buffer < 0 ||
      editor->current_buffer >= (int)editor->buffers.size()) {
...
bool LuaAPI::try_deliver_lsp_definition(
  ...
  if (pending_lsp_definition.empty()) return false;
  if (!editor || editor->buffers.empty() || editor->current_buffer < 0 ||
      editor->current_buffer >= (int)editor->buffers.size()) {
...
bool LuaAPI::try_deliver_lsp_symbols(
  ...
  if (pending_lsp_symbols.empty()) return false;
  if (!editor || editor->buffers.empty() || editor->current_buffer < 0 ||
      editor->current_buffer >= (int)editor->buffers.size()) {
  ...
  const FileBuffer &buf = editor->buffers[(size_t)editor->current_buffer];
  if (buf.filepath != symbols.filepath) return false;
...
static const char *lua_completion_kind_name(int kind) {
  switch (kind) {
  case 1: return "Text"; case 2: return "Method"; case 3: return "Function";
  case 4: return "Constructor"; case 5: return "Field"; case 6: return "Variable";
  case 7: return "Class"; case 8: return "Interface"; case 9: return "Module";
  case 10: return "Property"; case 11: return "Unit"; case 12: return "Value";
  case 13: return "Enum"; case 14: return "Keyword"; case 15: return "Snippet";
  case 16: return "Color"; case 17: return "File"; case 18: return "Reference";
  case 19: return "Folder"; case 20: return "EnumMember"; case 21: return "Constant";
  case 22: return "Struct"; case 23: return "Event"; case 24: return "Operator";
...
bool LuaAPI::try_deliver_lsp_completion(
  ...
  if (pending_lsp_completion.empty()) return false;
  if (!editor || editor->buffers.empty() || editor->current_buffer < 0 ||
      editor->current_buffer >= (int)editor->buffers.size()) {
  ...
  const FileBuffer &buf = editor->buffers[(size_t)editor->current_buffer];
  if (buf.filepath != filepath) return false;
...
void LuaAPI::events_unsubscribe_from_lua(lua_State *L) {
  ...
  if (it == event_subscribers_.end()) return;
  auto &vec = it->second;
...
void LuaAPI::emit_event_bus(
  ...
  if (!lua_initialized || !editor || !is_main_thread()) return;
  auto it = event_subscribers_.find(name);
  if (it == event_subscribers_.end() || it->second.empty()) return;
  lua_State *L = static_cast<lua_State *>(lua_state);
  const int top = lua_gettop(L);
...
void LuaAPI::emit_lsp_hover(const LSPHoverResult &hover) {
  if (!has_event_subscribers("lsp.hover")) return;
...
void LuaAPI::emit_lsp_definition(const LSPDefinitionResult &definition) {
  if (!has_event_subscribers("lsp.definition")) return;
...
void LuaAPI::emit_lsp_symbols(const LSPDocumentSymbolResult &symbols) {
  if (!has_event_subscribers("lsp.symbols")) return;
...
void LuaAPI::emit_lsp_completion(
    const std::string &filepath,
    const std::vector<LSPCompletionItem> &items) {
  if (!has_event_subscribers("lsp.completion")) return;
...
void LuaAPI::emit_git_refreshed() {
  if (!editor || !has_event_subscribers("git.refreshed")) return;
...
void LuaAPI::push_filetree_children(lua_State *L) {
  ...
  std::function<void(const FileNode &)> search =
      [&](const FileNode &node) {
        ...
        for (const FileNode &child : node.children) search(child);
      };
  for (const FileNode &node : editor->file_tree) search(node);
  if (!found) return;
  int n = 1;
...
void LuaAPI::viewport_scroll_top_from_lua(lua_State *L) {
  if (!editor || editor->panes.empty()) return;
  const SplitPane &pane = editor->get_pane();
  if (pane.buffer_id < 0 ||
      pane.buffer_id >= (int)editor->buffers.size()) {
    return;
...
void LuaAPI::emit_buffer_event(const std::string &event,
  ...
  if (!has_event_subscribers(event)) return;
...
void LuaAPI::emit_diagnostics_changed(
  ...
  if (!has_event_subscribers("diagnostics.changed")) return;
...
void LuaAPI::emit_theme_switched(const std::string &name) {
  if (!has_event_subscribers("theme.switched")) return;
...
void LuaAPI::timer_clear_from_lua(lua_State *L) {
  ...
  if (editor) editor->event_loop_.cancel_timer(id);
...
void LuaAPI::fire_timer(int ref) {
  if (!lua_initialized || !lua_state) return;
  lua_State *L = static_cast<lua_State *>(lua_state);
  uint64_t timer_id = 0;
  bool repeat = false;
  for (const auto &kv : timer_entries_) {
  ...
  if (!repeat) {
    ...
    if (it == timer_entries_.end()) return;
    if (editor) editor->event_loop_.cancel_timer(timer_id);
    const std::string key = "timer." + std::to_string(ref);
    auto cb = lua_callbacks.find(key);
    if (cb != lua_callbacks.end()) {
...
void LuaAPI::push_theme_palette(lua_State *L) {
  ...
  auto color = [&](const char *slot, int fg, int bg) {
    ...
    if (bg >= 0) lua_push_int_field(L, "bg", bg);
...
void LuaAPI::emit_debugger_state_changed() {
  if (!editor || !has_event_subscribers("debugger.state_changed")) return;
  std::string sig;
  ...
  if (sig == last_debugger_sig_) return;
...
void LuaAPI::debugger_request_from_lua(lua_State *L, int kind) {
  ...
  if (lua_isnumber(L, 2)) target = (int)luaL_checkinteger(L, 2) - 1;
  ...
  case 0:
    ...
    if (st.active_thread_id > 0) client->stack_trace(st.active_thread_id);
    break;
  case 1:
    pending_debugger_variables = id;
    if (st.active_frame_id > 0) client->scopes(st.active_frame_id);
    break;
  default:
...
void LuaAPI::motion_from_lua(lua_State *L, int kind) {
  if (!editor) return;
  switch (kind) {
  case 0: editor->move_word_forward(false); break;
  case 1: editor->move_word_backward(false); break;
  case 2: editor->move_to_line_smart_start(false); break;
  case 3: editor->move_to_line_end(false); break;
  case 4: editor->move_to_file_start(false); break;
  case 5: editor->move_to_file_end(false); break;
  case 6: editor->jump_to_matching_bracket(); break;
  case 7: editor->select_current_function(); break;
  default: return;
  }
  editor->needs_redraw = true;
}
...
void LuaAPI::lsp_restart_all_from_lua(lua_State *L) {
  if (editor) editor->restart_all_lsp_clients();
}
