...
namespace
{
  std::vector<std::filesystem::path> theme_dirs()
  {
    ...
    const char *cfg = getenv("JOT_CONFIG_HOME");
    fs::path root = cfg && *cfg
                        ? fs::path(cfg)
                        : (app && *app ? fs::path(app) / "jot"
                                       : (home ? fs::path(home) / ".config" / "jot" : fs::path()));
    if (!root.empty())
    {
...
bool LuaAPI::apply_theme_file(const std::string &name, std::vector<std::string> &stack)
{
  for (const auto &dir : theme_dirs())
  {
    ...
    if (std::regex_search(text, base_match, base_re) && base_match[1].str() != name)
    {
      const std::string base = base_match[1].str();
      if (std::find(stack.begin(), stack.end(), base) == stack.end() && stack.size() < 8)
      {
    ...
    std::regex group("\\\"([^\"]+)\\\"\\s*:\\s*\\{([^}]*)\\}");
    for (std::sregex_iterator i(text.begin(), text.end(), group), e; i != e; ++i)
    {
      set_theme_color((*i)[1].str(), value((*i)[2].str(), "fg"), value((*i)[2].str(), "bg"));
    }
    return true;
  }
  return false;
...
void LuaAPI::set_theme_color(std::string name, int fg, int bg)
{
  ...
  else if (name == "keyword_storage" || name == "keyword.storage" || name == "keyword_modifier"
  ...
  else if (name == "function_method" || name == "method" || name == "function.method")
  {
  ...
  else if (name == "constant_macro" || name == "constant.macro" || name == "constant_predefined")
  {
  ...
  else if (name == "punctuation_bracket" || name == "punctuation.bracket")
  {
    mark_syntax_pair(SyntaxThemeSlot::PunctuationBracket);
    set_pair(theme.fg_punctuation_bracket, theme.bg_punctuation_bracket);
  }
  else if (name == "punctuation_delimiter" || name == "punctuation.delimiter")
  {
    mark_syntax_pair(SyntaxThemeSlot::PunctuationDelimiter);
    set_pair(theme.fg_punctuation_delimiter, theme.bg_punctuation_delimiter);
  }
  ...
  else if (name == "line_num" || name == "line_number" || name == "fg_line_num"
           || name == "bg_line_num")
  {
  ...
  else if (name == "status_message" || name == "statusline_message" || name == "fg_status_message")
  {
    set_fg(theme.fg_status_message);
  }
  else if (name == "status_logo" || name == "statusline_logo" || name == "fg_status_logo"
           || name == "bg_status_logo")
  {
    set_pair(theme.fg_status_logo, theme.bg_status_logo);
  }
  else if (name == "status_file" || name == "statusline_file" || name == "fg_status_file"
           || name == "bg_status_file")
  {
    set_pair(theme.fg_status_file, theme.bg_status_file);
  }
  else if (name == "status_info" || name == "statusline_info" || name == "fg_status_info"
           || name == "bg_status_info")
  {
    set_pair(theme.fg_status_info, theme.bg_status_info);
  }
  else if (name == "status_warning" || name == "status_warn" || name == "statusline_warning"
           || name == "fg_status_warning" || name == "bg_status_warning")
  {
    set_pair(theme.fg_status_warning, theme.bg_status_warning);
  }
  else if (name == "status_error" || name == "statusline_error" || name == "fg_status_error"
           || name == "bg_status_error")
  {
    set_pair(theme.fg_status_error, theme.bg_status_error);
  }
  else if (name == "status_muted" || name == "statusline_muted" || name == "fg_status_muted"
           || name == "bg_status_muted")
  {
    set_pair(theme.fg_status_muted, theme.bg_status_muted);
  }
  else if (name == "command" || name == "cmdline" || name == "fg_command" || name == "bg_command")
  {
    set_pair(theme.fg_command, theme.bg_command);
  }
  else if (name == "search_match" || name == "search" || name == "incsearch"
  ...
  else if (name == "sidebar_selected" || name == "sidebar_sel" || name == "fg_sidebar_selected"
           || name == "bg_sidebar_selected")
  {
  ...
  else if (name == "tab_active" || name == "tabline_sel" || name == "fg_tab_active"
           || name == "bg_tab_active")
  {
    set_pair(theme.fg_tab_active, theme.bg_tab_active);
  }
  else if (name == "tab_inactive" || name == "tabline" || name == "fg_tab_inactive"
           || name == "bg_tab_inactive")
  {
  ...
  else if (name == "terminal_tab_plus" || name == "fg_terminal_tab_plus"
           || name == "bg_terminal_tab_plus")
  {
    set_pair(theme.fg_terminal_tab_plus, theme.bg_terminal_tab_plus);
  }
  else if (name == "terminal_tab_separator" || name == "fg_terminal_tab_separator")
  {
  ...
  else if (name == "fg_keyword_storage" || name == "fg_keyword_modifier")
  {
    mark_syntax_fg(SyntaxThemeSlot::KeywordStorage);
    set_fg(theme.fg_keyword_storage);
  }
  else if (name == "fg_keyword_preproc" || name == "fg_keyword_preprocessor")
  {
  ...
  else if (name == "fg_function_constructor" || name == "fg_constructor")
  {
