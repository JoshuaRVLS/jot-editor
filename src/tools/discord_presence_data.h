...
// (src/data/languages.json at 6d5fb18), MIT licensed:
...
namespace jot_discord
{
  ...
  inline constexpr NameAsset kNameAssets[] = {
      ...
      {"/webpack(\\.dev|\\.development|\\.prod|\\.production)?\\.config(\\.babel)?\\.(js|jsx|"
       "coffee|ts|json|json5|yaml|yml)/i",
       "webpack"},
      ...
      {".cjsx", "coffee"},
      {".coffee.ecr", "coffee"},
      {"/contenthook\\.config\\.(ts|cjs|mjs|js)$/i", "contenthook"},
      {".coffee.erb", "coffee"},
      {".litcoffee", "coffee"},
      {".iced", "coffee"},
      {".cos", "cosmo"},
      ...
      {"/\\.(glsl|vert|frag|geom|tesc|tese|comp|rgen|rint|rahit|rchit|rmiss|rcall|mesh|task|vsh|"
       "fsh|gsh)$/i",
       "glsl"},
      {"/\\.hx(ml)?$/i", "haxe"},
      {"/^procfile/i", "heroku"},
      ...
      {"/^(Appraisals|(Rake|[bB]uild|Cap|Danger|Deliver|Fast|Guard|Jar|Maven|Pod|Puppet|Snap)file("
       "\\.lock)?)$/",
       "ruby"},
      ...
      {"/^(\\.?bash(rc|[-_]?(profile|login|logout|history|prompt))|_osc|config|install-sh|PKGBUILD)"
       "$/i",
       "shell"},
      {"/\\.(ksh|mksh|pdksh)$/i", "shell"},
      {".sh-session", "shell"},
      {"/\\.zsh(-theme|_history)?$|^\\.?(antigen|zpreztorc|zlogin|zlogout|zprofile|zshenv|zshrc)$/"
       "i",
       "shell"},
      {"/\\.fish$|^\\.fishrc$/i", "shell"},
      {"/^\\.?(login|profile)$/", "shell"},
  ...
  inline constexpr LanguageAsset kLanguageAssets[] = {
      {"abap", "text"},
      {"ansible", "ansible"},
      {"asciidoc", "asciidoc"},
      {"bat", "bat"},
  ...
  static_assert(sizeof(kLanguageAssets) / sizeof(kLanguageAssets[0]) > 0);
} // namespace jot_discord
