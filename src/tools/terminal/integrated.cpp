...
namespace
{
  ...
  IntegratedTerminal::StyledCell styled_from_vterm_cell(const VTermScreen *screen,
                                                        const VTermScreenCell &cell)
  {
    ...
    return {cell_text(cell), fg, bg, fg_default, bg_default, cell.attrs.reverse != 0};
  }
  ...
  int screen_sb_popline(int, VTermScreenCell *, void *)
  {
    return 0;
  }
...
void IntegratedTerminal::vterm_output_callback(const char *s, size_t len, void *user)
{
  auto *term = static_cast<IntegratedTerminal *>(user);
  if (term)
  {
  ...
  if (!handled)
  {
    ...
    if (ch >= 32)
    {
      if (is_alt)
      {
        ...
        if (!queue_input((const char *)&esc, 1))
          return false;
      }
...
void IntegratedTerminal::reset_scroll()
{
  scroll_offset = 0;
}
...
std::vector<std::string> IntegratedTerminal::get_recent_lines(int max_lines) const
{
  ...
  const int total =
      (int)scrollback.size() + (screen ? rows : 0) + (!screen && !current_line.empty() ? 1 : 0);
