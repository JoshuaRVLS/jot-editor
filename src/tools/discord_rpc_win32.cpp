...
DiscordRPC::DiscordRPC()
    : sockfd_(-1), state_(DISCONNECTED), started_at_(time(nullptr)), last_heartbeat_ms_(0),
      last_presence_update_ms_(0), last_connect_attempt_ms_(0), heartbeat_interval_ms_(30000),
      nonce_counter_(0)
{
}
...
bool DiscordRPC::write_all(const uint8_t *, size_t)
{
  return false;
}
bool DiscordRPC::send_frame(int, const std::string &)
{
  return false;
}
bool DiscordRPC::read_frame(int &, std::string &)
{
  return false;
}
