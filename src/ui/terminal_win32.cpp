...
Event Terminal::read_event()
{
  ...
  while (processed < 256 && peek_has_console_input())
  {
    ...
    if (record.EventType == KEY_EVENT)
    {
      ...
      {
        // Shift / Alt bare events were never surfaced either.
        continue;
      }
    }
    if (record.EventType == KEY_EVENT && record.Event.KeyEvent.bKeyDown)
    {
      ...
      // A key-down record with neither a virtual-key code nor a character
      // is how some byte-delivery consoles represent control bytes such as
      // ESC. If it is dropped, the rest of a mouse report ('[', 'M',
      // payload) falls through as typed text. Treat it as an Escape
      // candidate: a following report assembles normally, and the drain
      // tail emits a real Escape when nothing follows. Genuine keys always
      // carry a VK code, so this never swallows real input.
      ...
      if (keycode == 27)
      {
        // An Escape may be the start of a VT sequence whose bytes the
        ...
        // batch, so this adds no perceptible delay. Ctrl+Esc / Alt+Esc are
        // consumed by the OS (Start menu / window switch) and never reach a
        // console app, so no modifier check is needed.
        g_vt_state = 1;
        g_vt_state_since_ms = GetTickCount64();
        continue;
      }
...
{
  // NOTE: this must be requested even under conpty. Windows Terminal only
  // forwards mouse input once the full-tracking mode (1003) has been
  // requested -- 1002/1006 alone produce no reports at all, so clicks and
  // the wheel would be dead. The resulting reports can arrive as raw byte
  // records; the VT byte assembler in read_event() converts them into
  // mouse events, and the escape-sequence guard below keeps them from
  // ever being typed as text.
  buffer += "\x1b[?1003h";
  flush();
}
