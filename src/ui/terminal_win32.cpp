...
namespace
{
  ...
  // Whether no-button hover tracking (\x1b[?1003h) can be requested safely.
  // Classic conhost converts it into MOUSE_EVENT records, so it is harmless
  // there. Windows Terminal / ConEmu (conpty) instead deliver the always-on
  // no-button motion flood back as raw VT byte records -- ESC [ M <payload>
  // -- whose bytes leak into the editor as typed text ("[MYS", "[MC", ...)
  // no matter how they are parsed. Button and wheel interactions (1000/1002/
  // 1006) are converted to clean MOUSE_EVENT records by conpty, so those
  // sequences are still requested; only hover (1003) is skipped under
  // conpty, which also means no report bytes are generated by free mouse
  // movement.
  bool vt_hover_safe()
  {
...
void Terminal::enable_mouse_hover()
{
  ...
  // requested only when vt_hover_safe().
  if (vt_hover_safe())
  {
    buffer += "\x1b[?1003h";
    flush();
...
void Terminal::disable_mouse_hover()
{
  if (!vt_hover_safe())
  {
    return; // hover was never requested under conpty; do not enable it now
  }
  buffer += "\x1b[?1003l";
  buffer += "\x1b[?1002h";
  buffer += "\x1b[?1006h";
